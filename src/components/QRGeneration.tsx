...
export default function QRGeneration({ batchId, totalContacts, onGenerationComplete }: QRGenerationProps) {
  ...
  const getStatusInfo = () => {
    ...
    const status = (batchData as any).status;
    switch (status) {
      case "generating":
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center space-x-3">
            <span className="text-sm text-gray-500">
              {(batchData as any)?.processedContacts || 0} of {totalContacts} processed
            </span>
            <Button
              onClick={handleGenerate}
              disabled={isGenerating || (batchData as any)?.status === "completed"}
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {(batchData as any).contacts.slice(0, 10).map((contact: any) => (
                  <tr key={contact.id}>
